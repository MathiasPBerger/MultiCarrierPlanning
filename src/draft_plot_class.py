...
class Plot(object):
    ...
    def plot(self, folder, start=0, end=-1, start_zoom=4000, end_zoom=4500, figs=[]):
        ...
        P_PtB = [self.model.P_PtB[t].value for t in time]
        ...
        K_B = [self.model.K_B.value for t in time]
        K_B_dis = [-self.model.K_B.value*self.model.rho_B.value for t in time]
        ...
        gas_cons = [self.model.pi_NG[t]+self.model.P_NGtP[t].value for t in time]
        heat = [self.model.pi_NG[t] for t in time]
        heat_CH4tNG = [self.model.pi_NG[t] + self.model.P_CH4tNG[t].value for t in time]
        ...
        if 4 in figs:
            ...
            ax2.fill_between(time, [nuclear_prod[t]+renewable_prod[t]+PH_prod_pos[t]+NG_prod[t]+H2_prod[t]+disp_prod[t]+trs_im[t] for t in time], total_prod, alpha=0.3, label="ENS")
        ...
        if 9 in figs:
            ...
            ax1 = plt.subplot(3, 1, 1)
            ax1.plot(time,total_prod,'w',label="Solar + Wind + PHtP + Disp + H2 + NG + ENS")
            ...
            ax1.fill_between(time, [nuclear_prod[t]+renewable_prod[t]+PH_prod_pos[t]+NG_prod[t]+H2_prod[t]+disp_prod[t] for t in time], [nuclear_prod[t]+renewable_prod[t]+PH_prod_pos[t]+NG_prod[t]+H2_prod[t]+disp_prod[t]+trs_im[t] for t in time], alpha=0.3, label="Interconnection")
            ax1.fill_between(time, [nuclear_prod[t]+renewable_prod[t]+PH_prod_pos[t]+NG_prod[t]+H2_prod[t]+disp_prod[t]+trs_im[t] for t in time], [nuclear_prod[t]+renewable_prod[t]+PH_prod_pos[t]+NG_prod[t]+H2_prod[t]+disp_prod[t]+trs_im[t]+P_BtP[t] for t in time], alpha=0.3, label="Batt")
        ...
        if 21 in figs:
            ...
            ax2.plot(time, P_BtP, 'b',label="P_BtP")
            ...
            ax2.fill_between(time, P_BtP, 0, alpha=0.3, color="blue",label="P_BtP")
