...
class Plot(object):
    ...
    def plot(self, folder, start=0, end=-1, start_zoom=4000, end_zoom=4500, figs=[]):
        ...
        load = list(asarray(list(self.data.pi_L.values())))[start:end]
        ...
        disp_prod = [self.model.P_disp[t].value for t in time]
        H2_prod = [self.model.P_H2[t].value for t in time]
        NG_prod = [self.model.P_NG[t].value for t in time]
        trs = [self.model.P_trs[t].value for t in time]
        trs_im = [self.model.P_trs_im[t].value for t in time]
        trs_ex = [self.model.P_trs_ex[t].value for t in time]
        ENS = [self.model.Delta_P[t].value for t in time]
...
#        kappa_NGS_dis = [-self.model.kappa_NGS.value for t in time]
        ...
        total_prod = [
            nuclear_prod[t] + solar_prod[t] + w_on_prod[t] + w_off_prod[t] + PH_prod_pos[t] + disp_prod[t] + H2_prod[
                t] + NG_prod[t] + ENS[t] + trs_im[t] + P_BtP[t] for t in time]

        heat = [self.model.pi_NG[t] for t in time]
        CH4tNG = [self.model.P_CH4tNG[t].value for t in time]
        power = [self.model.P_NG_PP[t].value for t in time]
        heat_CH4tNG = [heat[t] + CH4tNG[t] for t in time]
        gas_cons = [heat[t] + CH4tNG[t] + power[t] for t in time]
#        heat_power_NGS = [self.model.pi_NG[t] + self.model.P_NGtP[t].value + self.model.P_NGtNGS[t].value for t in time]
#        CH4tNG_im = [-self.model.P_CH4tNG[t].value - self.model.P_NG_im[t].value for t in time]
#        CH4tNG_im_NGS = [-self.model.P_NGStNG[t].value - self.model.P_CH4tNG[t].value - self.model.P_NG_im[t].value for
#                         t in time]
#        CH4tNG_im_NGS_ENS = [
#            -self.model.P_NGStNG[t].value - self.model.P_CH4tNG[t].value - self.model.P_NG_im[t].value -
#            self.model.Delta_P_NG[t].value for t in time]
        ...
        kappa_disp_max = self.data.kappa_disp
        kappa_disp_max_vec = [kappa_disp_max for t in time]
        kappa_disp_min = self.data.mu_disp * self.data.kappa_disp
        kappa_disp_min_vec = [kappa_disp_min for t in time]
        kappa_NG_0 = self.data.kappa_NG_0
        kappa_NG = self.model.K_NG.value
        kappa_NG_vec = [kappa_NG_0[t] + kappa_NG for t in time]
        ...
        el_price = [1000 * self.model.theta_el[t] for t in time]
        ...
        if 4 in figs:
            ...
            ax2.fill_between(time, nuclear_prod, [nuclear_prod[t] + solar_prod[t] for t in time], alpha=0.3,
                             label="Solar")
            ax2.fill_between(time, [nuclear_prod[t] + solar_prod[t] for t in time],
                             [nuclear_prod[t] + solar_prod[t] + w_on_prod[t] for t in time], alpha=0.3,
                             label="Onshore Wind")
            ax2.fill_between(time, [nuclear_prod[t] + solar_prod[t] + w_on_prod[t] for t in time],
                             [nuclear_prod[t] + renewable_prod[t] for t in time], alpha=0.3, label="Offshore Wind")
            ax2.fill_between(time, [nuclear_prod[t] + renewable_prod[t] for t in time],
                             [nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] for t in time], alpha=0.3,
                             label="PHtP")
            ax2.fill_between(time, [nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] for t in time],
                             [nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] + NG_prod[t] for t in time],
                             alpha=0.3, label="NG")
            ax2.fill_between(time, [nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] + NG_prod[t] for t in time],
                             [nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] + NG_prod[t] + H2_prod[t] for t in
                              time], alpha=0.3, label="H2")
            ax2.fill_between(time,
                             [nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] + NG_prod[t] + H2_prod[t] for t in
                              time], [nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] + NG_prod[t] + H2_prod[t] +
                                      disp_prod[t] for t in time], alpha=0.3, label="Disp")
            ax2.fill_between(time, [
                nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] + NG_prod[t] + H2_prod[t] + disp_prod[t] for t in
                time], [nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] + NG_prod[t] + H2_prod[t] + disp_prod[t] +
                        trs_im[t] for t in time], alpha=0.3, label="Imports")
            ax2.fill_between(time, [
                nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] + NG_prod[t] + H2_prod[t] + disp_prod[t] + trs_im[
                    t] for t in time], [nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] + NG_prod[t] + H2_prod[t] +
                                        disp_prod[t] + trs_im[t] + P_BtP[t] for t in time], alpha=0.3, label="BtP")
            ax2.fill_between(time, [
                nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] + NG_prod[t] + H2_prod[t] + disp_prod[t] + trs_im[
                    t] + P_BtP[t] for t in time], total_prod, alpha=0.3, label="ENS")
        ...
        if 5 in figs:
            ...
            ax3.plot(time, E_CH4, 'b',label="CH4 Storage")
            ax3.fill_between(time, 0, E_CH4, alpha=0.3, label="CH4 Storage SOC")
        ...
        if 9 in figs:
            ...
            ax1.fill_between(time, nuclear_prod, [nuclear_prod[t] + solar_prod[t] for t in time], alpha=0.3,
                             label="Solar")
            ax1.fill_between(time, [nuclear_prod[t] + solar_prod[t] for t in time],
                             [nuclear_prod[t] + solar_prod[t] + w_on_prod[t] for t in time], alpha=0.3,
                             label="Onshore Wind")
            ax1.fill_between(time, [nuclear_prod[t] + solar_prod[t] + w_on_prod[t] for t in time],
                             [nuclear_prod[t] + renewable_prod[t] for t in time], alpha=0.3, label="Offshore Wind")
            ax1.fill_between(time, [nuclear_prod[t] + renewable_prod[t] for t in time],
                             [nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] for t in time], alpha=0.3,
                             label="PHtP")
            ax1.fill_between(time, [nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] for t in time],
                             [nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] + NG_prod[t] for t in time],
                             alpha=0.3, label="NG")
            ax1.fill_between(time, [nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] + NG_prod[t] for t in time],
                             [nuclear_prod[t] + renewable_prod[t] + PH_prod_pos[t] + NG_prod[t] + H2_prod[t] for t in
                              time], alpha=0.3, label="H2")
        ...
        if 10 in figs:
            ...
            ax2.fill_between(time, P_H2_out_m, P_H2tP_m, alpha=0.3,label="P_H2tCH4")
        ...
        if 12 in figs:
            ...
            #plt.plot(time, gas_cons, color="orange", label="Gas Consumption")
            plt.plot(time, kappa_NGNet_vec, "r", label="Linepack Capacity")
            plt.fill_between(time, 0, heat, label="Heating")
            plt.fill_between(time, heat, heat_CH4tNG, label="CH4tNG")
            plt.fill_between(time, heat_CH4tNG, gas_cons, color="orange", label="NG PP")
        ...
        if 13 in figs:
            ...
            ax3.plot(time, P_PHtP, 'b',label="P_PHtP")
            ax3.fill_between(time, P_PHtP, 0, alpha=0.3, color="blue",label="P_PHtP")
        ...
        if 14 in figs:
            ...
            ax2.plot(time, P_PHtP, 'b',label="P_PHtP")
            ax2.fill_between(time, P_PHtP, 0, alpha=0.3, color="blue",label="P_PHtP")
        ...
        if 22 in figs:
            ...
            fig22 = plt.figure()
            fig22.suptitle("RES - Wholesale Electricity Prices", fontweight="bold")
            ax1 = plt.subplot(4, 1, 1)
            ...
            if self.save_fig:
                fig22.savefig(dir_plots + "/elprice_plot.pdf")
                plt.show()
